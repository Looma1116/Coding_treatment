...
void getNode(node **p)
{
    ...
    (*p)->next = NULL;
    (*p)->prev = NULL;
    (*p)->distance = 0; //대역폭을 얻어야 하므로 작은놈들
    (*p)->visited = 0;
}
void getEdge(edge **e)
{
...
node *removeMax() //그래프가 우선순위큐이기 때문에 헤더 바로 다음거 리턴
{
    node *tmp;
    tmp = G->next;
    G->next = tmp->next;
...
{
    node *tmp;
    while (p->prev != G && p->prev->distance < p->distance) //만약 해당 노드거리가 이전 노드의 거리보다 크면
    {
        tmp = p->prev;
        tmp->next = p->next;
        if (p->next != NULL)
    ...
    while (p->next != NULL && p->next->distance > p->distance) //반대로 작으면
    {
        tmp = p->next;
        p->prev->next = tmp;
        tmp->prev = p->prev;
